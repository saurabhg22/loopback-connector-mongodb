...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  ...
  } else if (value instanceof mongodb.ObjectID || mongodb.ObjectID.isValid(value.toString? value.toString() : '')) { // instanceof method might return false
    return true;
  } else if(Array.isArray(value)) { // Added this block
    return value.every( item => mongodb.ObjectId.isValid(item) )
  } else {
    return false;
  }
}
